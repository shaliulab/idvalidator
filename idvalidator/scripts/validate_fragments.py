...
def get_parser():
    ...
    ap.add_argument(
        ...
        type=str,
        required=True,
        help="Path to single idtrackerai results folder",
    )
    ap.add_argument(
        "--ncores", "-j", type=int, default=1, help="Number of cores to use"
    )
    return ap
...
def validate(experiment_folder, ncores=1):
    ...
    corrections = pd.read_csv(
        os.path.join(experiment_folder, "corrections.csv")
    )
    ...
    if ncores == 1 or len(folders) == 1:
        ...
        joblib.Parallel(n_jobs=ncores)(
            joblib.delayed(validate_single_thread)(
                corrections, os.path.join(experiment_folder, folder)
            )
            for folder in folders
        )
