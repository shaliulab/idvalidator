...
def main(ap=None, args=None):
    ...
    session_folder = os.path.join(args.experiment_folder, session_name)
    experiment_name = os.path.basename(args.experiment_folder.rstrip("/"))
    ...
    problem_frames = np.where(np.diff([0] + missing) > 0)

    time_window_length = 5 # seconds

    store, lowres_store = load_store(args.experiment_folder, args.chunk)
...
def check_feed(blobs_in_video, timestamp, output_folder, colors=None, stores=None, experiment_folder=None, chunk=None):
    ...
    chunk_first_frame = store._get_chunk_metadata()["frame_number"][0]
